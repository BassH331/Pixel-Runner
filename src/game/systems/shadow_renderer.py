...
class ShadowRenderer:
    ...
    def render_entity_shadow(
        ...
        squash_ratio: Optional[float] = None,
        fade_height: Optional[float] = None,
        ground_snap: Optional[float] = None,
        y_offset: Optional[int] = None,
        scale_mult: Optional[float] = None,
    ) -> None:
        """
        Render a ground-projected silhouette shadow beneath an entity.
        If parameters are omitted, automatically loads the configured profile from ShadowRegistry.
        ...
        Args:
            ...
            scale_mult: Shadow width multiplier.
        """
        ...
        # Resolve configuration profile from registry if arguments are None
        profile = ShadowRegistry.get_profile(entity)
        eff_alpha = base_alpha if base_alpha is not None else profile.alpha
        eff_squash = squash_ratio if squash_ratio is not None else profile.squash_ratio
        eff_fade = fade_height if fade_height is not None else profile.fade_height
        eff_snap = ground_snap if ground_snap is not None else profile.ground_snap
        eff_y_off = y_offset if y_offset is not None else profile.y_offset
        ...
        temp_w = max(4, int(orig_w * eff_scale_mult))
        temp_h = max(2, int(orig_h * eff_squash))
        _, orig_feet_y, _ = cls.get_silhouette_shadow(image, temp_w, temp_h, eff_alpha)
        ...
        if ground_y is not None:
            ...
            if air_height <= eff_snap:
                scale_factor = 1.0
                effective_ground_y = true_feet_y
            elif air_height >= eff_fade:
                return
            else:
                scale_factor = 1.0 - (air_height / eff_fade)
        ...
        if isinstance(depth_scale, (int, float)):
            scale_factor *= float(depth_scale)

        if scale_factor < 0.05:
            return

        shadow_w = max(4, int(orig_w * eff_scale_mult * scale_factor))
        shadow_h = max(2, int(orig_h * eff_squash * scale_factor))
        current_alpha = int(eff_alpha * min(1.0, scale_factor))
        ...
        shadow_y = int(effective_ground_y - shadow_feet_bottom + eff_y_off)

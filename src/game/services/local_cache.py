...
class LocalCache:
    """Thread-safe local SQLite cache for configurations and pending telemetry.
    
    Uses standard library sqlite3 and zlib compression to ensure there are no
    external dependencies, with threading locks and WAL journal mode to prevent
    database locks during multi-threaded background telemetry sync.
    """
    ...
    def _initialize(cls) -> None:
        """Initialize database folder, WAL mode, and schema."""
        if cls._initialized:
            return
        try:
            ...
            # Enable WAL mode for concurrent multi-thread read/writes

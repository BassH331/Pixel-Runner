...
class Wizard(pg.sprite.Sprite):
    def __init__(self, x, y, screen_width, scale=1.0, start_delay=0.0):
        ...
        # Movement & State
        self.speed = 180 # Slightly slower walk
    ...
    def load_animations(self):
        ...
        self.animator.add("idle", Animation(idle_frames, 0.15))
        
        # Run (Move)
        run_frames = load_and_scale(f"{base_path}/Move")
        self.animator.add("run", Animation(run_frames, 0.15))
    ...
    def update(self, dt):
        ...
        if self.state == "WAITING":
            ...
            # Walk to center from left
            self.rect.x += self.speed * dt_sec
            self.animator.set("run")
            self.animator.flip_x = False # Face right
            
            if self.rect.centerx >= self.screen_width // 2 - 200: # Stop earlier than others to lead
                self.state = "IDLE"
                self.state_timer = 0
            ...
            self.state_timer += dt_sec
            if self.state_timer >= 1.0: # Wait 1.0s (longer pause for wizard)
                self.state = "ATTACK"

...
Displays sequential tutorial steps on top of gameplay, each showing:
 ...
 - Text + animated key sprites on the right (showing which buttons to press)
 - Oracle tower animating in a small side-banner (narrator)
...
──────────────────────
All positions are pre-computed in ``_build_layout()`` into a simple dict of
*named pixel coordinates*.  The ``draw()`` method never does maths — it just
reads ``self._pos["sprite_center"]``, ``self._pos["title_topleft"]``, etc.

To move something, find its name in ``_build_layout()`` and change the
pixel numbers.  Every position is a ``(x, y)`` tuple or a plain ``int``.
...
class TutorialOverlay:
    ...
    def __init__(
        ...
        *,
        ...
        banner_width: float  = 0.55,     # main banner width  (fraction of screen)
        oracle_width: float  = 0.18,     # oracle banner width (fraction of screen)
        sprite_scale: int    = 3,        # player animation scale multiplier
        oracle_scale: int    = 2,        # oracle tower scale multiplier
        key_scale: int       = 4,        # key icon scale multiplier

        # ── POSITIONS ───────────────────────────────────────────────────────
        #    Where each element sits.  All values are fractions of the screen
        #    (0.0 = left/top, 1.0 = right/bottom) unless noted otherwise.
        banner_x: float      = 0.50,     # banner horizontal centre
        banner_y: float      = 0.50,     # banner vertical centre
        oracle_x: float      = 0.89,     # oracle banner horizontal centre
        oracle_y: float      = 0.22,     # oracle banner vertical centre
        ...
        sprite_x: float      = 0.25,     # player sprite horizontal centre
        sprite_y: float      = 0.50,     # player sprite vertical centre
        title_x: float       = 0.65,     # title horizontal centre
        title_y: float       = 0.20,     # title vertical position (top edge)
        text_x: float        = 0.65,     # description / keys horizontal centre
        text_wrap_width: float = 0.40,   # max text width (fraction of banner)
        prompt_y: float      = 0.90,     # prompt vertical position (bottom edge)
        counter_x: float     = 0.94,     # step counter horizontal (right-aligned)
        ...
        desc_color: tuple    = (230, 220, 200),
        prompt_color: tuple  = (200, 190, 170),
        counter_color: tuple = (180, 170, 150),
        ...
        # Scale the banner images (height is auto-calculated from aspect ratio)
        ...
        ow = int(sw * oracle_width)
        oh = int(ow * raw_banner.get_height() / raw_banner.get_width())
        self._oracle_banner = pg.transform.smoothscale(raw_banner, (ow, oh))
        ...
        # Main banner — centre on screen
        self._pos["banner_topleft"] = (
            int(sw * banner_x) - bw // 2,
            int(sh * banner_y) - bh // 2,
        )
        bx, by = self._pos["banner_topleft"]

        # Oracle sub-banner — centre at given screen fraction
        self._pos["oracle_topleft"] = (
            int(sw * oracle_x) - ow // 2,
            int(sh * oracle_y) - oh // 2,
        )
        # Clamp oracle to screen edges
        ox, oy = self._pos["oracle_topleft"]
        ox = min(ox, sw - ow - 5)
        oy = max(oy, 5)
        self._pos["oracle_topleft"] = (ox, oy)
        self._pos["oracle_center"] = (ox + ow // 2, oy + oh // 2)

        # Player sprite — positioned within the banner
        self._pos["sprite_center"] = (
            bx + int(bw * sprite_x),
            by + int(bh * sprite_y),
        )

        # Title text — positioned within the banner
        self._pos["title_centerx"] = bx + int(bw * title_x)
        self._pos["title_top"]     = by + int(bh * title_y)

        # Description / keys — horizontal centre within the banner
        self._pos["text_centerx"]  = bx + int(bw * text_x)
        self._pos["text_max_width"] = int(bw * text_wrap_width)

        # Prompt — bottom of banner
        self._pos["prompt_centerx"] = bx + bw // 2
        self._pos["prompt_bottom"]  = by + int(bh * prompt_y)

        # Step counter — bottom-right of banner
        self._pos["counter_right"]  = bx + int(bw * counter_x)
        self._pos["counter_bottom"] = by + int(bh * prompt_y)

        # ── Store rects for blitting ────────────────────────────────────────
        self._banner_rect = pg.Rect(bx, by, bw, bh)
        self._oracle_rect = pg.Rect(ox, oy, ow, oh)
    ...
    def draw(self, surface: pg.Surface) -> None:
        ...
        if frames:
            ...
            rect = sprite.get_rect(center=p["sprite_center"])
            surface.blit(sprite, rect)

        # ── 4. Title (right side, top) ──────────────────────────────────────
        ...
        # ── 5. Description text (below title) ──────────────────────────────
        ...
        for line_surf in desc_lines:
            lr = line_surf.get_rect(centerx=p["text_centerx"], top=y)
            surface.blit(line_surf, lr)
            y += line_surf.get_height() + self._desc_line_gap

        # ── 6. Key icons (pulsing, below description) ──────────────────────
        ...
        # ── 7. "Press KEY to continue" prompt ──────────────────────────────
        ...
        # ── 9. Oracle tower sub-banner ─────────────────────────────────────
        surface.blit(self._oracle_banner, p["oracle_topleft"])
        if self._oracle_frames:
            oracle_sprite = self._oracle_frames[min(self._oracle_frame, len(self._oracle_frames) - 1)]
            surface.blit(oracle_sprite, oracle_sprite.get_rect(center=p["oracle_center"]))
